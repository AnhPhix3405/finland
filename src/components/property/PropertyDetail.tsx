...
export function PropertyDetail({ type, listing, isDemo = false }: PropertyDetailProps) {
  ...
  // Mock data for the demo property or use real listing data
  const property = useMemo(() => {
    ...
    if (listing) {
      return {
        ...
        area: listing.area || 0,
        bedrooms: 0, // This could be extracted from description or tags
        floors: 0, // This could be extracted from description or tags
        ...
        tags: listing.tags || [],
        propertyType: listing.property_types?.name || "Dự án",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="relative aspect-video rounded-xl overflow-hidden bg-slate-200 dark:bg-slate-800">
            {property.images && property.images.length > 0 ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-slate-400">
                ...
                <span className="ml-2">Không có ảnh</span>
              </div>
            )}
          </div>

          <div className="grid grid-cols-3 gap-2">
            {property.images && property.images.length > 1 && property.images.slice(1, 4).map((img: string, idx: number) => (
          ...
          <div className="flex flex-wrap gap-2 pt-4">
            {property.hashtags && property.hashtags.map((tag: string) => (
              <span 
                key={tag}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 sticky top-24 shadow-sm">
            <div className="flex items-center gap-4 mb-6">
              <div className="relative size-14 rounded-full overflow-hidden shrink-0">
                <Image 
                  src={(property.broker as any).avatar_url || (property.broker as any).avatar || "https://i.pravatar.cc/150?u=default"}
                  alt={(property.broker as any).full_name || (property.broker as any).name || "Broker"}
                  fill 
                  className="object-cover" 
                />
              </div>
              <div>
                ...
                <h4 className="font-bold text-slate-900 dark:text-white leading-tight">{(property.broker as any).full_name || (property.broker as any).name}</h4>
